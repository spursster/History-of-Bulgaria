...
// Генерираме страниците от pages.json
...
// Генериране на seals.html
function generateSealsPage() {
  try {
    const sealsPath = path.join(__dirname, 'data', 'seals.json');
    if (!fs.existsSync(sealsPath)) {
      console.warn('⚠️ seals.json не е намерен. Пропускам seals.html.');
      return;
    }
...
<body>
  <div id="page">
    ...
    fs.writeFileSync(path.join(__dirname, 'seals.html'), html, 'utf8');
    console.log('✅ Генерирана страница: seals.html');
  } catch (err) {
    console.warn('⚠️ Грешка при генериране на seals.html:', err.message);
  }
}

// Генериране на coins.html
function generateCoinsPage() {
  try {
    const coinsPath = path.join(__dirname, 'data', 'coins.json');
    if (!fs.existsSync(coinsPath)) {
      console.warn('⚠️ coins.json не е намерен. Пропускам coins.html.');
      return;
    }
...
<body>
  <div id="page">
    ...
    fs.writeFileSync(path.join(__dirname, 'coins.html'), html, 'utf8');
    console.log('✅ Генерирана страница: coins.html');
  } catch (err) {
    console.warn('⚠️ Грешка при генериране на coins.html:', err.message);
  }
}
